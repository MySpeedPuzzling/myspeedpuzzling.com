...
- **The write model works with PHP objects** — a handler loads entities through repository methods (`get()`, `findBy…()`), changes them through their public properties or named entity methods, and removes them through a repository `delete()`. No DQL `UPDATE`/`DELETE` and no native SQL in handlers or services; the one exception is a genuine bulk operation over rows that must not be loaded one by one (e.g. `LentPuzzleTransfer` rewrite in `ApprovePuzzleMergeRequestHandler`, prune crons), and it says why in a comment. An `#[Immutable]` property is changed only through a named method on its entity that explains the case (e.g. `PuzzleChangeRequest::proposedManufacturerMergedInto()`)
...
- **Getting started (newcomer onboarding)**: `docs/features/getting-started-guide.md` — name asked at registration; `/welcome` = one question + big buttons (native *and* social sign-up land there); slim `finish_profile` page reusing `EditProfileFormType`/`EditProfile`; "Getting started" card on the Hub + the player's own profile = 5 steps **derived from data** (`GetGettingStartedProgress`, nothing stored), newcomers only (14 days), hidden via `HintType::GettingStartedChecklist`, auto-dismissed by `HubController` when complete, `<details>` open until the first puzzle is logged, folded afterwards; public guide `getting_started` in **all 6 locales** (no screenshots, anchors pinned by test, linked from footer/user menu/FAQ). Deliberately no tour/modal — research in the doc
...
- **Community moderators**: `docs/features/community-moderators.md` — non-admin role (`player.moderator_since`, null = none) that reviews the puzzle catalogue (change + merge requests, puzzle approvals) and reaches nothing else under `/admin`. `PuzzleModerationVoter` (`PUZZLE_MODERATION_ACCESS` = admin OR moderator); the access_control rule `^/admin/puzzle-((change|merge)-requests|approvals)` must stay above `^/admin`. Admins appoint/revoke at `/admin/moderators` (`GrantModeratorRole`/`RevokeModeratorRole`, player picker reused from event maintainers). Moderator-reachable areas outside the catalogue get their own capability attribute
- **Puzzle approvals**: `docs/features/puzzle-approvals.md` — queue `/admin/puzzle-approvals` (admins + moderators, `PUZZLE_MODERATION_ACCESS`) for `puzzle.approved = false`. **No reject** (new puzzles carry solving times): approve with corrections (`ApprovePuzzle`), or merge into the duplicate via the existing merge review (moderator = reporter, `?return=` back; the survivor inherits approval). New brand → `PuzzleApprovalBrandChoice` approve / use existing / merge into an approved brand (moves all its puzzles + change-request proposals as objects, deletes the duplicate). **Every catalogue decision (change/merge approve+reject, puzzle/brand approval, brand merge) writes an append-only `puzzle_moderation_decision` row** via `PuzzleModerationDecisionRecorder` in the deciding handler — no FKs, decider id/name/code copied, so it outlives requests, puzzles and players; a new decision type must record there too
...
- **Pairs & teams**: `docs/features/pairs-and-teams/README.md` + `implementation-plan.md` — every pair/team result belongs to a `puzzling_team`: **the exact set of people** (order irrelevant, no rosters; identity = `composition_key`, the only implementation is `Value\TeamComposition`; guests keyed by normalised name). `puzzle_solving_time.puzzling_team_id` is **always set together with the `team` JSON snapshot** (constructor / `modify()`); the JSON stays the display snapshot, so old read queries are untouched and select the column as `team_id`. `PuzzlingTeamResolver` = lookup + one-statement `INSERT … ON CONFLICT DO NOTHING` outside the unit of work (race-safe; call it *after* every validation of the handler). A team never changes members — editing a time's group moves the time; `DeletePlayerHandler` turns the member into a guest in place (`PuzzlingTeamMemberConversion`, merges on key collision; the member FK is RESTRICT on purpose). Name = optional, shared, public, ≤ 50 chars, any *registered* member renames (`RenamePuzzlingTeam`, others get `PuzzlingTeamRenamed` notification), the add form may only name an unnamed team (`team_name` field → `nameIfUnnamed`). **A team with results can never be deleted** (`DeletePuzzlingTeam` → 409). Add-form picker: `_copuzzler_picker.html.twig` + `copuzzler_picker_controller.js` — hidden `group_players[]` inputs are the single source of truth (wire format unchanged), per-mode stash so switching Solo/Pair/Team never loses anything, nothing in it may submit/re-render/navigate (Enter in the search included), suggestions fetched lazily from `/{_locale}/my-co-puzzlers.json` so **a solo form pays no query**; kill switch `PAIRS_TEAMS_PICKER_PUBLIC` (1 = public; 0 = old co-puzzler rows for everybody but admins). Pages: `/{_locale}/pairs-and-teams` (manage), `/{_locale}/teams/{id}` (public, stats members-only, `noindex` when unnamed), profile "with…" filter + puzzle page "My pairs/teams only" (both filter in PHP, zero queries). One-off backfill: `myspeedpuzzling:backfill-puzzling-teams` (keyset batches, idempotent). **Guests** (Guests tab of the manage page): `PuzzlingTeamMemberConversion` is the only code that changes who a member is — `playerToGuest` (deletion), `renameGuest`, `guestToPlayer`; it merges on key collision and rewrites the group snapshots; a guest becomes a player **only with that player's consent** (`GuestLinkRequest` → notification → `/pairs-and-teams/guest-link/{id}`, GET only shows, answer is a POST; a player who blocks the asker is never asked and the asker cannot tell). **Archive** = one member's own "keep out of my shortcuts" (`puzzling_team_archive`), touches nothing else, undone automatically inside the resolver's lookup when the same people puzzle again. `myspeedpuzzling:cleanup-empty-puzzling-teams` (manual) removes empty unnamed leftovers. API result rows carry `team_id` + `team_name` (additive; never rename API fields, `solo`/`duo`/`team` stay). Team routes require a UUID so `/pairs-and-teams/guests/...` is not read as a team id. **Boot migrations run under a Postgres advisory lock** (`bin/migrate-database`, `MigrationLock`, fails open) — two blue-green containers take turns
- **Free trial**: `docs/features/free-trial/README.md` — 10 days of membership, once per player, no Stripe, live for everyone (no flag). **A trial is just a `membership.granted_until` grant**, so every membership gate works unchanged; eligibility = *no `membership` row* (a row only ever comes from a subscription, a voucher, an admin grant or a trial) and the unique index on `player_id` makes "once" race-free. `StartFreeTrial` → `Membership::startFreeTrial()`; `trial_*` columns exist for the UI and the funnel only. **Startable only when the account is 7 days old** (`FreeTrial::MINIMUM_ACCOUNT_AGE_DAYS`; `PlayerProfile::canStartFreeTrial()` for the UI, `StartFreeTrialHandler` authoritative; no activity condition on purpose - players who never log a time do become members) - a younger account is told the date instead (`_free_trial_unlock_sentence.html.twig`). Offered on the membership page, in `#membersExclusiveModal` and by a one-time modal (button + modal only once startable); web only (store review). Checkout already turns remaining grant days into a Stripe trial ("subscribe now, keep your days" - except in the last 24 h). Reminder 3 days before the end: cron `myspeedpuzzling:send-free-trial-ending-reminders` (daily). Numbers at `/admin/free-trial`. Trial players count as `active_trials`, not `active_members`
- **Player code history**: `player_code_change` (previous/new code, when) - written by `EditPlayerCodeHandler` on every real change, cascades with the player. A log only, nothing reads it yet; a personal code is a member feature a free trial hands out too
- **Announcement modals**: `docs/features/announcement-modals.md` — modals the site opens by itself, **once per player ever**: `AnnouncementModal` enum + a template per case that stays in the code + one `AnnouncementModalRule` service each (no queries in rules - they run on every page view), picked by `ResolveAnnouncementModal` (full-page GET only, quiet routes, no `?return=`, web only, 14-day pause between modals). **Never twice = the impression is claimed before rendering** (`INSERT .. ON CONFLICT DO NOTHING` into `player_modal_impression`; only the winning request renders); the browser's `seen_at` report is for counting only. `announcement_modal()` claims - `base.html.twig` is its only caller. `player.modal_displayed` is dead and gets dropped one release later (blue-green)
