...
- **Remember me**: always on, no checkbox — sliding 30 days, signature-based on `['email','password']`. Enabled ahead of Phase 6 by `RememberMeMigrationWindowPass`, which (a) swaps the firewall's remember-me listener for `MigrationWindowRememberMeListener` because core's clears the cookie on *every* `LoginFailureEvent` and the window-era Auth0 authenticator fails on every request, and (b) pins the handler's user provider to `user_account_provider` (the chain provider would `JsonException`/500 for a deleted account holding a valid cookie). Both are deleted in Phase 6 — see `docs/features/auth-migration/implementation-plan.md` §2b
- **Auth0 authenticator, window A**: it claims *every* request carrying a session cookie and fails on every native session, so its failure must never produce a Response — a Response from any authenticator short-circuits the request. `MigrationWindowAuth0Authenticator` wraps it on the firewall and returns `null` from `onAuthenticationFailure()` whenever a token is already present. Without it, any `access_control` pattern that is not `PUBLIC_ACCESS` sends signed-in users to `/login` and on to their profile — which is what made the whole `/admin` area unreachable for admins (fixed 2026-08-11). Corollary: `^/admin` asks for `ADMIN_ACCESS`, never `IS_AUTHENTICATED_FULLY` — the latter also rejects a `RememberMeToken`. Deleted in Phase 6
...
- **Puzzle Picker ("What should I solve next?")**: `docs/features/puzzle-picker/README.md` — design draft (2026-08-18, not implemented): seeded-random picker over the player's shelf with free core filters + members-only Insights filters (prediction gap, difficulty, predicted time), compact card with my times + prediction, predictions toggle in collections; read model = one two-phase raw-SQL query (measured ≤ 75 ms worst case on prod), bulk `GetPlayerPredictions` instead of a materialised table; staged PRs in `implementation-plan.md`
