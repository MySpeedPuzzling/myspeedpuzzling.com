...
- **Return URLs**: `docs/features/return-url.md` — `?return=` / `?return_title=` convention. **Every consumer must validate via `SpeedPuzzling\Web\Value\ReturnUrl::tryFrom()`** (rejects `//`, `/\`, schemes, encoded variants, CRLF) or the Twig `safe_return_url()`; templates propagate values onward unsanitised by design, so validation at the point of *use* is the invariant. Plan doc also covers the pending move of post-login redirects off the session
