...
- **Free trial**: `docs/features/free-trial/README.md` — 10 days of membership, once per player, no Stripe, behind `FREE_TRIAL_ENABLED` (OFF). **A trial is just a `membership.granted_until` grant**, so every membership gate works unchanged; eligibility = *no `membership` row* (a row only ever comes from a subscription, a voucher, an admin grant or a trial) and the unique index on `player_id` makes "once" race-free. `StartFreeTrial` → `Membership::startFreeTrial()`; `trial_*` columns exist for the UI and the funnel only. Offered on the membership page, in `#membersExclusiveModal` (no age gate) and by a one-time modal; web only (store review). Checkout already turns remaining grant days into a Stripe trial ("subscribe now, keep your days" - except in the last 24 h). Reminder 3 days before the end: cron `myspeedpuzzling:send-free-trial-ending-reminders` (daily). Numbers at `/admin/free-trial`. Trial players count as `active_trials`, not `active_members`
