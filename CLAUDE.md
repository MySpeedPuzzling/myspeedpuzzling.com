...
- **Computation model (hybrid — NOT batch-only)**: per-entity metrics update in (near) real time; only global/percentile metrics are batch-only.
  - **Synchronous, in-request**: `PuzzleSolved` / `PuzzleSolvingTimeModified` / `PuzzleSolvingTimeDeleted` are routed to the `sync` transport (`config/packages/messenger.php`) and dispatched by `DomainEventsSubscriber` (Doctrine `postFlush`). `RecalculateIncrementalPuzzleIntelligenceOnSolvingTimeChange` upserts the affected player **baseline**, puzzle **difficulty**, and player **improvement ratios** (solo solves only); `RecalculatePuzzleStatisticsOnSolvingTimeChange` upserts `puzzle_statistics`.
  - **Asynchronous**: that handler dispatches `RecalculateDerivedMetricsForPuzzle` (routed `async`), which recomputes a puzzle's derived metrics except memorability.
  - **Batch every 15 min**: `myspeedpuzzling:recalculate-puzzle-intelligence` (`PuzzleIntelligenceRecalculator`) fully recomputes everything in dependency order and is the ONLY place that produces the global/percentile metrics — **player skill tiers/percentiles, MSP Rating (ELO), skill history, rating snapshots, memorability normalization, global improvement ratios** — plus stale-row cleanup.
  - **Testing/fixtures caveat**: because `PuzzleSolved` runs `sync`, loading fixtures or adding times recomputes baseline+difficulty immediately. Do NOT hardcode `puzzle_difficulty` rows in fixtures (they get overwritten on the next flush); a puzzle's difficulty only becomes non-null once ≥5 of its solvers have a `player_baseline` for that piece count.
- **Services**: All calculation logic is in `src/Services/PuzzleIntelligence/` — `PlayerBaselineCalculator`, `PuzzleDifficultyCalculator`, `PlayerSkillCalculator`, `DerivedMetricsCalculator`, `MspRatingCalculator`, `ImprovementRatioCalculator`, `PuzzleIntelligenceRecalculator` (batch orchestrator). Real-time handlers live in `src/MessageHandler/Recalculate*OnSolvingTimeChange.php` + `RecalculateDerivedMetricsForPuzzleHandler`.
