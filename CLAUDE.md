...
- **Puzzle merges are destructive and audited:** approving deletes the merged puzzles and moves their solving times/collection/wishlist/sell-swap/lending rows onto the survivor. `ApprovePuzzleMergeRequestHandler` therefore writes a `puzzle_merge_audit` row first (full before/after snapshot + the ids of everything migrated or dropped, plus `decisionSource`/`decisionConfidence`/`decisionNote`) — for admin-UI merges too. It also carries over any EAN, catalogue number, alternative name, cover image or manufacturer that **only** a to-be-deleted puzzle had, so a merge never silently loses product data. Note the admin UI still *pre-fills* multiple EANs as a comma-joined string — pick one real value, don't post the join.
