# Subject: Hero spot illustration

Compact scene displayed to the right of the homepage headline (contained illustration, NOT a background).
...
CRITICAL — jigsaw piece anatomy: every puzzle piece must be the classic die-cut shape, exactly like the standard puzzle-piece icon (🧩): a rounded square where each side is either flat, has ONE centered round knob protruding on a narrow neck, or ONE matching round socket cut inward. Maximum one knob or socket per side; knobs are mushroom-shaped (narrow neck, round head). Never wavy edges, never blobby outlines, never multiple bumps on a side. If pieces are shown assembled, seams are thin dark-navy cut lines following those knob-and-socket shapes. Prefer FEW, LARGE pieces; render in flat vector illustration style with clean, SVG-like linework.

Subject: A compact, balanced scene of speed puzzling in action: two hands over a nearly assembled white-and-coral jigsaw puzzle on a small round table, with exactly ONE piece-shaped hole remaining and one hand snapping the final coral piece toward it, short navy speed dashes trailing the moving piece. Beside the puzzle, one round stopwatch (coral button, sky-blue hands) standing upright. One single oversized lavender puzzle piece leans against the table leg. Nothing else — generous white space around the scene.
