...
**Status (2026-07-30):** Decisions locked 2026-07-23 (staged rollout, D14–D17 below; **D15 amended 2026-07-25** — the login-page modal is dropped). **Phase 2 build is COMPLETE**: 2a (data model), 2b (security plumbing), 2c-I (login page + magic sign-in link), 2c-II (registration, verification, password reset, change password/email) and 2d (code sweep — chokepoint, `#[CurrentUser]` sweep, test helpers, audit logging, Stage B banner wording; remember-me deferred to Phase 6, see implementation-plan §2b) are done; 2a–2c verified dark on production. The announcement newsletter went out 2026-07-30 naming the cutover week **Aug 3–8**. Next: **Stage A** (registration flip + Auth0 signup freeze + hash-export ticket), then Stage B when the export lands.
