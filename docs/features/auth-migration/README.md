...
| Remember me | **Deferred to Phase 6** (2026-07-30): Symfony's `RememberMeListener` clears the cookie on every `LoginFailureEvent` unconditionally, and the window-era Auth0 authenticator fails on every anonymous request — enabling remember-me stamps a deletion cookie on every anonymous response and breaks #164 shared cacheability. Ships (signature-based, `secure: true`, `samesite: lax`) when the Auth0 authenticator leaves the firewall | core (config commented in `security.php`) |
