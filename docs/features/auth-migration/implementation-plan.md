...
- [ ] **Scrub credentials out of Sentry before Stage B** (found in the 2c-I slice): `config/packages/sentry.php` sets `send_default_pii: true` and leaves `max_request_body_size` at its default, so any exception raised while handling `POST /login` ships the plaintext password in the event's request body. Harmless today (nothing posts there yet), unacceptable once the native form is live — add a `before_send` that strips `password`/`plainPassword`/`_csrf_token` from request data, and verify with a deliberate 500 on the login POST.
