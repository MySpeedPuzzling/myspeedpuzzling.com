...
- **Flag:** env var `XP_SYSTEM_ADMIN_ONLY` → parameter `xpSystemAdminOnly` (`config/services.php`) → `SpeedPuzzling\Web\Services\Xp\XpFeatureGate` — `isVisibleFor(?PlayerProfile)` restricts visibility to admins; `isEmailSendingEnabled()` suppresses ALL feature emails (achievement congratulations, weekly digest, reveal emails) for everyone while active
- **Default:** **ON** (`.env` ships `XP_SYSTEM_ADMIN_ONLY=1`, and the gate's constructor default is admin-only too, so a missing env can never leak the feature). The test env inherits the ON baseline — the leak tests assert the flagged side. Local dev flips it in `.env.local` (`XP_SYSTEM_ADMIN_ONLY=0`) to see every surface as a non-admin and let the feature emails reach Mailpit
- **Gated files** (authoritative checklist in `docs/features/xp-levels/leak-inventory.md`):
  ...
  - Templates that only LINK to the feature (`/ladder` CTA + `_ladder_dropdown`, footer, `player_profile`): the `xp_system_visible()` Twig function from `src/Twig/XpTwigExtension.php`
  - Controllers: `BadgesOverviewController`, `AchievementDetailController`, `AchievementInfoController`, `XpLeaderboardController`, `XpHistoryController`, `XpExplainerController`, `FairPlayXpController`, `XpLaunchRevealController`, `XpShareCardController`, `RevealBadgeController`, `BadgeRevealsController`, `EditTimeController` (delete-dialog XP warning), `EditProfileController` (digest + opt-out form fields)
...
- **Remove when:** XP launch day (`docs/features/xp-levels/launch-runbook.md`) — flip the env var to `0` first (no deploy needed, instant rollback), then remove the gate + call sites, DELETE the flag tests (`tests/Controller/XpLeakTest.php`, `XpPagesTest.php`, `XpSurfacesTest.php`, `DigestSettingsVisibilityTest.php`, flag cases in `BadgesOverviewControllerTest` + `RecalculateBadgesForPlayerHandlerTest`), then run the reveal-email command
